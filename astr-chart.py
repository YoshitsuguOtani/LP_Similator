...
def get_historical_astar_data():
    try:
        # Set date range
        end_date = datetime.now()
        start_date = end_date - timedelta(days=365)
        
        # CoinGecko API endpoint for Astar (ASTR)
        url = f"https://api.coingecko.com/api/v3/coins/astar/market_chart/range"
        params = {
            "vs_currency": "usd",  # Change to your preferred currency if needed
            "from": int(start_date.timestamp()),
            "to": int(end_date.timestamp())
        }
        
        print("Fetching Astar (ASTR) data...")
...
def plot_multiple_charts(df):
    ...
    ax1.set_title('Astar (ASTR) Price Trend (Line)')
    ax1.set_xlabel('Date')
    ...
    # Adjust layout
    plt.suptitle('Astar (ASTR) Price Analysis (Last 365 Days)', fontsize=16, y=1.02)
    plt.tight_layout()
    
    # Save the figure as a .png file
    plt.savefig('astar_price_analysis.png', dpi=300)
    print("Plot saved as 'astar_price_analysis.png'")
